...
FPS = 50.0
OUTPUT = 'out%i%.mp4'
FOURCC = cv2.VideoWriter_fourcc(*'RAW ')  # noqa
...
cnt = 0
out = cv2.VideoWriter(
    filename=OUTPUT.replace('%i%', str(cnt)),
    apiPreference=cv2.CAP_ANY,
    fourcc=int(FOURCC),
    fps=float(FPS), frameSize=(win_size[0], win_size[1]), isColor=False
)
hg_text = 'Dummy Text'
...
try:
    while 1:
        ...
        if not get_win_size(game_hwnd) == win_size:
            win_size = get_win_size(game_hwnd)
            cnt += 1
            print('Window Size changed, using new output')
            out.release()
            out = cv2.VideoWriter(
                filename=OUTPUT.replace('%i%', str(cnt)),
                apiPreference=cv2.CAP_ANY,
